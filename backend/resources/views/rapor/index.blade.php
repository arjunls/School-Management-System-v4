...
<div class="space-y-6">
    ...
    <div class="grid gap-6 md:grid-cols-2">
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <h2 class="font-semibold text-slate-900 mb-4">Cetak Per Siswa</h2>
            <form action="{{ route('rapor.generate') }}" method="POST" class="space-y-4" target="_blank">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors text-sm">
                        <i class="fas fa-download mr-2"></i>Download PDF
                    </button>
                    <button type="submit" formaction="{{ route('rapor.preview') }}" class="px-6 py-2 bg-slate-200 text-slate-800 rounded-lg hover:bg-slate-300 transition-colors text-sm">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <h2 class="font-semibold text-slate-900 mb-4">Cetak Batch per Kelas</h2>
            <form action="{{ route('rapor.batch') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Kelas <span class="text-red-500">*</span></label>
                    <select name="class_id" required class="w-full px-4 py-2 bg-slate-50 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500">
                ...
                <div>
                    ...
                    <select name="term_id" required class="w-full px-4 py-2 bg-slate-50 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500">
                        <option value="">Pilih Semester</option>
                        @foreach($tahunAjaran as $ta)
                        <optgroup label="{{ $ta->name }}">
                            @foreach($ta->terms as $t)
                ...
                <button type="submit" class="px-6 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors text-sm">
                    <i class="fas fa-file-archive mr-2"></i>Download Batch (ZIP)
                </button>
            </form>
        </div>
    </div>
