            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900">{{ number_format($pendingPayments) }}</p>
                    </div>
                    <div class="bg-red-50 p-3 rounded-lg"><i class="fas fa-credit-card text-red-600 text-xl"></i></div>
                </div>
            </div>
    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-start space-x-3">
                        <div class="flex-shrink-0">
                            <div class="w-3 h-3 rounded-full" style="background: {{ $e->color ?? '#3b82f6' }}"></div>
                        </div>
                        <div class="flex-1">
                            <p class="font-medium text-slate-900">{{ $e->title }}</p>
                            <p class="text-sm text-slate-500">{{ \Carbon\Carbon::parse($e->start_date)->format('d M') }}@if($e->end_date) - {{ \Carbon\Carbon::parse($e->end_date)->format('d M') }}@endif</p>
                        </div>
                    </div>
                    @empty
                    <p class="text-sm text-slate-400">Belum ada acara mendatang</p>
                    @endforelse
                    <a href="{{ route('kalender.index') }}" class="block text-sm text-blue-600 hover:text-blue-800 font-medium mt-2">{{ __('dashboard.lihat_semua') }}</a>
                </div>
            </div>
        </div>
    </div>
