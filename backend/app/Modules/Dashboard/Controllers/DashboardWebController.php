...
class DashboardWebController extends Controller
{
    public function index()
    {
        ...
        $todayAttendance = AttendanceRecord::whereDate('date', today())->where('status', 'hadir')->count();
        $totalAttendance = AttendanceRecord::count();
        $pendingPayments = \App\Modules\Finance\Fee\Models\FeeInvoice::where('status', 'unpaid')->count();
        $totalTugas = \App\Modules\Learning\Assignment\Models\Assignment::count();
        $activePkl = \App\Modules\StudentLife\PKL\Models\PKLRecord::where('status', 'active')->count();

        $attendanceRate = $totalAttendance > 0
            ? round((AttendanceRecord::where('status', 'hadir')->count() / $totalAttendance) * 100, 1)
            : 0;

        // Upcoming events from Kalender
        $upcomingEvents = \App\Modules\Calendar\Models\Event::where('start_date', '>=', today())
        ...
        return view('dashboard.index', compact(
            ...
            'gradeLabels', 'gradeData', 'pendingPayments', 'totalTugas',
            'activePkl', 'upcomingEvents'
        ));
    }
}
