...
export function TermFormModal({ open, onClose, onSuccess, academicYearId, editing }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await academicYearAPI.updateTerm(editing.id, form);
        toast('Term updated', 'success');
      } else {
        await academicYearAPI.createTerm(academicYearId, form);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-lg bg-white rounded-lg shadow-lg p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">{editing ? 'Edit Term' : 'Add Term'}</h2>
