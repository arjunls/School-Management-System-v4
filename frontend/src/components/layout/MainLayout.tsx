    ...
    <div className="flex min-h-screen bg-background">
      ...
      <aside
        className={`
          ...
          bg-sidebar/70 backdrop-blur-xl border-r border-sidebar-border
      ...
      >
        {/* Logo */}
        <div className={`flex h-14 items-center gap-2 border-b border-sidebar-border ${sidebarCollapsed ? 'justify-center px-0' : 'px-4'}`}>
          <div className="flex size-8 items-center justify-center rounded-lg bg-gradient-to-br from-blue-500 to-blue-600 shrink-0 shadow-sm">
          ...
          {!sidebarCollapsed && (
            <div className="flex flex-col">
              <span className="text-sm font-semibold tracking-tight text-sidebar-foreground">SMK Nusantara</span>
              <span className="text-[10px] text-sidebar-foreground/50 tracking-wide">School Management</span>
            </div>
          )}
        </div>

        {/* Gradient divider */}
        <div className="h-px mx-3" style={{ background: 'linear-gradient(to right, transparent, var(--sidebar-border), transparent)' }} />

        {/* Nav */}
        <nav className="flex-1 overflow-y-auto overflow-x-hidden p-2 space-y-0.5 scrollbar-thin">
          {visibleNav.map((item) => {
            ...
            return (
              <Link
                ...
                className={`
                  relative flex items-center gap-3 rounded-md px-3 py-1.5 text-sm font-medium transition-all duration-200 overflow-hidden
                  ${sidebarCollapsed ? 'justify-center px-2' : ''}
                  ${isActive ? 'text-blue-300' : 'text-sidebar-foreground/60 hover:text-sidebar-foreground'}
                `}
                title={sidebarCollapsed ? item.label : undefined}
              >
                {isActive && (
                  <motion.div layoutId="nav-active" className="absolute inset-0 rounded-md bg-blue-500/10" transition={{ type: 'spring', stiffness: 300, damping: 30 }} />
                )}
                <span className="relative z-10 shrink-0">{item.icon}</span>
                {!sidebarCollapsed && <span className="relative z-10">{item.label}</span>}
                {isActive && !sidebarCollapsed && (
                  <motion.div layoutId="nav-bar" className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-r-full"
        ...
        <div className="p-2 space-y-0.5">
          {bottomItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href}
                className={`flex items-center gap-3 rounded-md px-3 py-1.5 text-sm font-medium transition-all duration-200 ${sidebarCollapsed ? 'justify-center px-2' : ''} text-sidebar-foreground/60 hover:bg-sidebar-accent/60 hover:text-sidebar-foreground ${isActive ? 'bg-sidebar-accent text-sidebar-accent-foreground' : ''}`}
                title={sidebarCollapsed ? item.label : undefined}
              >
          ...
          <button onClick={handleLogout}
            className={`w-full flex items-center gap-3 rounded-md px-3 py-1.5 text-sm font-medium transition-all duration-200 ${sidebarCollapsed ? 'justify-center px-2' : ''} text-sidebar-foreground/50 hover:bg-destructive/10 hover:text-destructive`}
        ...
        {/* User profile */}
        <div className={`border-t border-sidebar-border p-3 ${sidebarCollapsed ? 'flex justify-center' : ''}`}
          style={{ borderImage: 'linear-gradient(to right, transparent, var(--sidebar-border), transparent) 1' }}
        >
          <div className={`flex items-center gap-3 ${sidebarCollapsed ? 'flex-col' : ''}`}>
            <div className="flex size-8 items-center justify-center rounded-full text-xs font-medium text-white shadow-sm shrink-0"
            ...
            {!sidebarCollapsed && (
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-sidebar-foreground/50 capitalize truncate">{userRole}</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-border bg-background/80 backdrop-blur-lg px-4 sm:px-6">
          ...
          <button onClick={() => setSidebarCollapsed(!sidebarCollapsed)}
            className="hidden lg:flex p-1.5 rounded-md text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors"
          ...
          <div className="w-1 h-5 rounded-full shrink-0 hidden sm:block" style={{ backgroundColor: sectionColor }} />
          ...
          <nav className="hidden sm:flex items-center gap-1 text-xs text-muted-foreground">
            ...
            {pathname !== '/dashboard' && pathname.split('/').filter(Boolean).map((segment, i, arr) => (
              <React.Fragment key={segment}>
                ...
                <span className={i === arr.length - 1 ? 'text-foreground font-medium' : 'hover:text-foreground transition-colors'}>
          ...
          <div className="relative">
            <button onClick={() => setSearchOpen(!searchOpen)}
              className="p-1.5 rounded-md text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors sm:hidden"
            >
              <svg className="size-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
              </svg>
            </button>
            <div className="hidden sm:flex items-center gap-2 rounded-md border border-input/50 bg-muted/30 px-3 py-1.5 text-sm text-muted-foreground w-48 lg:w-56 transition-all duration-200 focus-within:border-primary/50 focus-within:bg-background focus-within:shadow-xs">
              <svg className="size-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
              </svg>
              <span className="text-muted-foreground/60">Cari...</span>
            </div>
          </div>

          {/* Dark mode toggle */}
          <button onClick={toggleDark}
            className="p-1.5 rounded-md text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-all duration-200"
            title={dark ? 'Mode Terang' : 'Mode Gelap'}
          >
            <motion.div key={dark ? 'dark' : 'light'} initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} transition={{ duration: 0.2 }}>
              {dark ? (
